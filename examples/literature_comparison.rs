    ...
    v.mapv(|val| {
        if val < gamma {
            val.powi(p) * (p as f64 * PI / 2.0).cos()
                / (2.0 * PI * gamma * (gamma * gamma - val * val).sqrt()
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &p in &[1, 4] {
        ...
        let dynamical_error = ht.iter().zip(expected_ht.iter()).map(|(&h, &e)| {
            20.0 * ((e - h).abs() / ht_max).log10()
        }).collect::<Array1<f64>>();
    ...
    // First calculate e_1 and e_2 for 1024 points
    let error_2: f64 = expected_ht.iter().zip(ht.iter()).map(|(&e, &h)| (e - h).abs()).sum::<f64>() / ht.len() as f64;
    let error_1: f64 = func.iter().zip(retrieved_func.iter()).map(|(&f_val, &r_val)| (f_val - r_val).abs()).sum::<f64>() / func.len() as f64;
    ...
    let error_2_512: f64 = expected_ht_512.iter().zip(ht_512.iter()).map(|(&e, &h)| (e - h).abs()).sum::<f64>() / ht_512.len() as f64;
    let error_1_512: f64 = func_512.iter().zip(retrieved_func_512.iter()).map(|(&f_val, &r_val)| (f_val - r_val).abs()).sum::<f64>() / func_512.len() as f64;
