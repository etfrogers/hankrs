...
fn bench_creation(c: &mut Criterion) {
    ...
    for size in [256, 1024].iter() {
        group.bench_with_input(criterion::BenchmarkId::from_parameter(size), size, |b, &n| {
            b.iter(|| {
                HankelTransform::new(black_box(0), black_box(10.0), black_box(n))
            })
        });
    }
    group.finish();
...
fn bench_transforming(c: &mut Criterion) {
    ...
    for size in [256, 1024].iter() {
        ...
        group.bench_with_input(criterion::BenchmarkId::from_parameter(size), size, |b, &_n| {
            b.iter(|| {
                transformer.qdht(&f, Axis(0))
            })
        });
    }
    group.finish();
